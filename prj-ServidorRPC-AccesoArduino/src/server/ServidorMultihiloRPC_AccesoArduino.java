...
public class ServidorMultihiloRPC_AccesoArduino {
	private static InterfazAccesoArduino AA; // Se define la variable AA de tipo
												// interfaz de acceso a arduino
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (direcciones.length > 1) {
				...
				miIP = InetAddress.getLocalHost(); // miIP = direcciones[0];
		...
		System.out.println(
				"ServidorMultihiloRPC_AccesoArduino escuchando por el puerto 4000 y la IP: " + miIP.getHostAddress());
		System.out.println(
				"Enviará los mensajes de los clientes al puerto " + args[1] + " del arduino con IP: " + args[0]);
		...
		while (listening) {
			...
			try {
				...
				// EJERCICIO: y crear un hilo MultiServerThread para que la
				// gestione
